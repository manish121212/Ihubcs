 ...
 * @author sharmila
 */
public class SUNameReader {

	private static final Logger logger = Logger.getLogger(SUNameReader.class.getName());
		...
		try {
			...
			if (esbEnv == null) {
				...
				if (homeFolder != null && !homeFolder.equals("")) {
					esbEnv = homeFolder + File.separatorChar + "SUNameReader.properties";
					System.out.println("the ens " + esbEnv);
					logger.info("esbEnv: " + esbEnv);
				} else {
					esbEnv = "E:\\Projects\\Inspirage\\SUNameReader.properties";
				}
			}
			...
			Logger.getLogger(SUNameReader.class.getName()).log(Level.SEVERE,
			...
			if (fis != null) {
				try {
					...
					Logger.getLogger(SUNameReader.class.getName()).log(Level.SEVERE, null, ex);
				}
			}
		}
	}
	/**
	 * Returns display name against technical service names
	 * @param key technical service name in format clientid.servicename.split("-")[0]
